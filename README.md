...
> 文档版本：V1.4
> 最后更新：2026-09-12
> 变更记录：V1.4 将产品版本同步至 PRD V1.16；补充数据分析卡片入口与子页面、首页趋势收纳、详细记录渐进展示、最新测试基线与性能风险。
...
- 电脑与手机浏览器响应式布局。
- 已按 PRD V1.11 完成首页、用车记账、数据分析、车辆管理及“更多”的统一浅雾白／青绿色视觉系统；桌面端使用侧边导航，手机端使用五项底部导航。
- “更多”直接从“车辆与用车”“数据与备份”任务分组开始，使用三张仅包含真实状态摘要的入口卡片；支持车辆管理、能耗统计和数据管理，并覆盖空状态、返回路径与断点切换。
- 数据分析采用核心指标与三张独立入口卡片；费用趋势、月份费用对比和费用类别构成分别在独立子页面完整展示，并继承车辆、时间与类别条件。数据分析首页不再保留通用“查看详细记录”入口。
- 首页不再展示与数据分析重复的近六个月费用趋势；完整趋势统一由数据分析子页面承接。
- 详细记录已升级为渐进展示的记录中心：默认最新优先、首次显示 10 条、每次加载 10 条；搜索、排序摘要与筛选入口常驻，完整条件集中在筛选抽屉。
- 详细记录使用紧凑结果概览，桌面端为语义表格、手机端为单列卡片；编辑、复制与删除只在详情抽屉提供，并支持目标记录自动加载与短暂高亮。
- 详细记录区分无车辆、有车无记录、筛选无结果三类空状态；抽屉支持 Escape、焦点返回及 Tab 焦点循环。

详细产品范围及验收标准以 [PRD.md](./PRD.md) V1.16 为准。
...
截至 PRD V1.16，本轮迭代仅调整 React 组件、CSS、产品文档与测试；未引入网络服务、系统权限、数据库 schema 迁移、敏感配置或用户数据格式变更。
...
- Vitest：6 个文件、77 项通过。
- Playwright：41 项通过，覆盖桌面、iPhone、Galaxy、手机横竖屏、`767px` 断点、详细记录空状态与浏览器历史。
- `npm run build`：通过；当前存在主 JavaScript 包体超过 500 kB 的非阻塞 Vite 警告。
...
- Vite 构建当前主 JavaScript 包约 908 kB（gzip 约 268 kB），会提示超过 500 kB；这是非阻塞性能风险，后续应先评估路由分包和图表按需加载的收益、回归与包体预算，再决定是否实施。
