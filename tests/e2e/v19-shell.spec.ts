...
test('V1.9 mobile shell keeps the primary action and bottom navigation usable', async ({ page }) => {
  ...
  const home = bottomNavigation.getByRole('link', { name: '首页' })
  await expect(home).toHaveClass(/active/)
  await expect(home).toHaveCSS('background-color', 'rgb(223, 244, 242)')
  const calendar = bottomNavigation.getByRole('link', { name: '费用日历' })
  await calendar.click()
  await expect(calendar).toHaveClass(/active/)
  await expect(calendar).toHaveCSS('background-color', 'rgb(223, 244, 242)')
